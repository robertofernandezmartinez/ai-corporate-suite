...
# ============================================================
# FEATURE ENGINEERING: Respecting your original mapping & types
# ============================================================
class StockoutFeatureEngineer(BaseEstimator, TransformerMixin):
    def __init__(self):
        # Preservation of original feature order
        self.cat_cols = ["store_id", "category", "region", "weather", "holiday_promo", "seasonality", "month", "day_of_week", "product_id"]
        self.num_cols = ["inventory_level", "units_sold", "price", "discount", "competitor_pricing", "is_weekend"]

    def fit(self, X, y=None): return self

    def transform(self, X):
        df = X.copy()
        # Respecting your exact column mapping from the PRO version
        mapping = {
            "Date": "date", "Store ID": "store_id", "Product ID": "product_id", "Category": "category",
            "Region": "region", "Inventory Level": "inventory_level", "Units Sold": "units_sold",
            "Units Ordered": "units_ordered", "Price": "price", "Discount": "discount",
        ...
        # Consistent date handling with dayfirst=True
        ...
        # Hardened NaN handling (fillna 0 and Unknown)
...
# ============================================================
# PREDICTOR: Optimized for Proactive Telegram Alerts
# ============================================================
class StockoutPredictor:
    ...
    async def predict_from_file(self, file):
        ...
        try:
            ...
            # Security Shield: Checking mandatory fields before processing
            required = ["Product ID", "Inventory Level", "Units Sold", "Price"]
            missing = [c for c in required if c not in df_raw.columns]
            if missing:
                return {"success": False, "detail": f"Format Error. Missing columns: {missing}"}

            probabilities = self.pipeline.predict_proba(df_raw)[:, 1].astype(float)
            price = pd.to_numeric(df_raw.get("Price", 0), errors="coerce").fillna(0.0)
            units_sold = pd.to_numeric(df_raw.get("Units Sold", 0), errors="coerce").fillna(0.0)
            impact = (probabilities * price * units_sold).astype(float)
            ...
            results_df = pd.DataFrame({
                ...
                "batch_id": batch_id, 
                "created_at": now_ts,
                "product_id": df_raw["Product ID"].astype(str),
            ...
            if self.supabase: self._persist_batches(results_df, "stockout_predictions")

            # Structured summary for main.py Telegram trigger
            return {
                "success": True,
                "processed_records": len(results_df),
                "summary": {
                    "critical_risks": int((results_df["risk_level"] == "CRITICAL").sum()),
                    "total_exposure": float(impact.sum())
                },
                "batch_id": batch_id
            }
